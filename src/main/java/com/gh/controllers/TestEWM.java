...
public class TestEWM {
	...
	/*
	 * public static void main(String args[]) { int width=150; int height=150;
	 * String format="png"; String
	 * contents="https://sslwx.zrchn.com/a/b/deviceno=BP"+1234;
	 * HashMap<EncodeHintType, Comparable> map=new HashMap<EncodeHintType,
	 * Comparable>(); map.put(EncodeHintType.CHARACTER_SET, "utf-8");
	 * map.put(EncodeHintType.ERROR_CORRECTION,ErrorCorrectionLevel.M);
	 * map.put(EncodeHintType.MARGIN, 0); try { BitMatrix bm = new
	 * MultiFormatWriter().encode(contents, BarcodeFormat.QR_CODE, width, height);
	 * Path file=new File("D:/"+1234+".png").toPath();
	 * MatrixToImageWriter.writeToPath(bm, format, file); } catch (WriterException
	 * e) { e.printStackTrace(); } catch (IOException e) { e.printStackTrace(); } }
	 */
    
    //  导出word
	 @RequestMapping(value="/test")  
		public String test(Model model,HttpServletRequest request,String ids){
		 ...
		 WordUtil mdoc = new WordUtil();  
	        ...
	        mdoc.createDoc(dataMap, "F:/新 LED标签.doc");  
	        model.addAttribute("message","导出word成功");  
